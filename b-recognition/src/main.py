...
from utils import process_video, group_and_filter_id
...
def main(args, app, charbank_dict, anno_df, anno_df_with_face):
    ...
    movies_ids = anno_df['id'].unique().tolist() # Get unique movie IDs (e.g., 'tt1217613')
    all_profile_names = []

    # Create a mapping from movie_id (ttXXXXXX) to movie_title (e.g., '3009_BATTLE_LOS_ANGELES')
    # This assumes a unique movie title for each movie ID in anno_df
    movie_id_to_title = anno_df.set_index('id')['movie'].to_dict()

    for movie_id in movies_ids:
        movie_title = movie_id_to_title.get(movie_id)

        if movie_title and movie_title in charbank_dict:
            # Get the set of name IDs from charbank_dict for the current movie_title
            # Assuming 'charbank_nmids' is a list in the first row of the DataFrame
            charbank_pred_per_frame = charbank_dict[movie_title]
            current_movie_name_ids = set(charbank_pred_per_frame["charbank_nmids"].iloc[0])

            movie_profile_path = os.path.join(args.profile_dir, movie_id)
            if os.path.exists(movie_profile_path):
                for filename in os.listdir(movie_profile_path):
                    if filename.endswith(".jpg"):
                        # Extract the actor ID from the filename (e.g., 'nm0000154' from 'nm0000154.jpg')
                        actor_id = os.path.splitext(filename)[0]
                        if actor_id in current_movie_name_ids:
                            all_profile_names.append(filename)
    ...
    for anno_idx, anno_row in anno_df.iterrows():
        ...
        for char_info in cast_info:    
            ...
            if char_info['id'] + ".jpg" in all_profile_names:
                try:
                    image = np.array(Image.open(os.path.join(args.profile_dir, imdbid, char_info['id'] + ".jpg")).convert("RGB"))
                    faces = app.get(image)
                except:
        ...
        if len(cast_ids) == 0: # no profile face available for the cast list
            anno_df_with_face = anno_df_with_face._append(anno_row)
            continue
        else:
        ...
        anno_row["characters"] = index_filtered_dict
        anno_df_with_face = anno_df_with_face._append(anno_row)
...
if __name__ == "__main__":
    ...
    # load character bank
    TOP_K_CHARS = 10
    with open(args.charbank_path, "rb") as f:
        charbank_dict = pickle_compat.load(f)
        charbank_dict = {str(key):value for key,value in charbank_dict.items()}

    # load AD annotaion file
    anno_df = pd.read_csv(args.anno_path)
    anno_df["bboxes"] = [[[]] * 32] * len(anno_df)
    anno_df["characters"] = [{}] * len(anno_df)
    ...
    anno_df_with_face.to_csv(os.path.join(args.output_dir, f"anno_with_face_reproduced_{args.score_thresh}_{args.det_thresh}.csv"), index=False)